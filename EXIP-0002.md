...
2. Ditch the Simple Lookback Average algorithm and move to a Moving Average algorithm. After looking through performance of these algorithms, it seems that WWHM is doing the best in terms of stability vs responsiveness to load changes. Masari implements WWHM and fits exactly with out desired blocktimes, so it would be trivial to implement WWHM into Elixium. 
